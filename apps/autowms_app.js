var autowms_app = function (element, webmapjs) {
  ...
  var createFile = function (data) {
    ...
    if (data.leaf) {
      ...
      return (
        "<span class='autowms_app_fileitem autowms_app_diritem' name='" +
        data.name +
        "'>&#128448;&nbsp;" +
        name +
        "</span>"
  ...
  var createReturnLink = function (data, path) {
    ...
    html +=
      "<hr/><span class='autowms_app_currentpath'>Current folder: ./" +
      path +
      "</span>";
    html +=
  ...
  var makeGetCapabilitiesRequest = function (data, path) {
    ...
    var getcapabilitiesdone = function (layers) {
      ...
      html +=
        "<span class='autowms_app_serviceabstract'><b>" +
        service.title.replace(new RegExp("!", "g"), "<br/>") +
        "</b><hr/>" +
        service.abstract.replace(new RegExp("!", "g"), "<br/>");

      if (wmsServiceURL) {
        html +=
          '<hr/>WMS: <a target="_blank" href="' +
          wmsServiceURL +
        ...
        if (wmsServiceURL.split("?").length > 1) {
          ...
          html +=
            '<hr/>OpenDAP: <span class=\'autowms_app_opendaplinks\'><a target="_blank" href="' +
            dapURL +
            '">' +
            dapURL +
            "</a>";
