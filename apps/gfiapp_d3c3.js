var gfiapp_d3c3 = function (element, webmapjs) {
  var plotData = function (datatoplot, elementid) {
    ...
    $("#" + elementid).append(
      "<div style='height=40px;border=2px solid red;' class='controls gfiapp_d3c3_export_to_csv_button'>b</div>"
    );
    ...
    controls.button({ label: "Export to CSV" }).click(function () {
      ...
      link.setAttribute(
        "href",
        encodeURI(`data:text/csv;charset=utf-8,${csvExport}`)
      );
  ...
  var pointOnMapClicked = function (options) {
    ...
    for (var j = 0; j < layers.length; j++) {
      var gfiUrl = webmapjs.getWMSGetFeatureInfoRequestURL(
        layers[j],
        options.x,
        options.y
      );
  ...
  var init = function () {
    element.html(
      '<div id="info"></div><div id="chart" class="gfiapp_d3c3"></div>'
    );
