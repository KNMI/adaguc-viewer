...
ba=document.createElement("div"),Q=!0,p=new WMJSBBOX,D=new WMJSBBOX,ea=new WMJSBBOX,pa=new WMJSBBOX,O=new WMJSBBOX,S=[];k.getLegendStore=function(){return legendImageStore};var J=[],oa=!1,Za="default",Ba=!1,Ja="default",$a=!1,Ka=$('<div class="WMJSDivBuffer-loading"/>',{}),ab=0,da=0,ia=1,Ca,ra=0,V=0,Ma=0,Na=0,X=Array(3),Oa=Array(3);for(j=0;3>j;j++)X[j]=new WMJSProjection,Oa[j]=new WMJSProjection;var bb=!0,B=new WMJSListener;this.getGraphingData=function(){return this.graphingData};this.setGraphingData=
function(a){this.graphingData=a};var ga=function(ca){a.id||(a.id="WebMapJSMapNo_"+WebMapJSMapNo,WebMapJSMapNo++);return a.id+"_"+ca};this.enableInlineGetFeatureInfo=function(a){bb=a};this.closeAllGFIDialogs=function(){WMJSDialog.closeAllDialogs(U)};var U=[],cb=function(){enableConsoleDebugging&&console.log("onlegendready called");ja()},sa=[],Da=0,ta=!1,ja=function(a){if(!(2>S.length)){try{0==isDefined(a)&&(a=!1);if(1==ta&&isDefined(cb)){1==B.addToCallback("onlegendready",cb)&&enableConsoleDebugging&&
...
this.getLayers=function(){for(var a=[],b=0;b<t.length;b++)a.push(t[t.length-b-1]);return a};this.setLayer=function(a,b){return k.addLayer(a,b,a)};this.setActive=function(a){$a=a};this.setActiveLayer=function(a){Ca=a;ja();B.triggerEvent("onchangeactivelayer")};this.calculateNumBaseLayers=function(){T=0;if(F)for(var a=0;a<F.length;a++)F[a].enabled&&0==F[a].keepOnTop&&T++};this.enableLayer=function(a){a.enabled=!0;k.calculateNumBaseLayers();k.rebuildMapDimensions();ja(!0)};this.disableLayer=function(a){a.enabled=
...
height:C}};this.getWidth=function(){return z};this.getHeight=function(){return C};this.repositionLegendGraphic=function(a){Q?ja(a):(S[0].hide(),S[1].hide())};this.resizeHeight=this.resizeWidth=-1;var Ra=!1,nb=new WMJSTimer;this.setSize=function(a,b){enableConsoleDebugging&&console.log("setSize",a,b);4>parseInt(a)||4>parseInt(b)?console.log("Skipping setSize",a,b):(this.resizeWidth=parseInt(a),this.resizeHeight=parseInt(b),!1===Ra?(Ra=!0,k._setSize(k.resizeWidth,k.resizeHeight)):nb.init(200,function(){Ra=
...
1E3>d;){d++;for(var f=null,k=t.length-1;0<=k;k--){var g=t[k].getDimension("time");g&&(g=g.getNextClosestValue(c))&&"date too early"!==g&&(null===f||moment(g)<moment(f))&&(f=g)}if(null===f)break;e.push({name:"time",value:f});c=f}if(0<e.length)e.pop(),this.draw(e);else if(c=t[0].getDimension("time")){e=c.size();f=Math.min(c.size(),100);d=[];for(f=e-f;f<e;++f)d.push({name:c.name,value:c.getValueForIndex(f)});this.draw(d)}}};var ob=new WMJSDebouncer,Ta=!1,Ua=!1,Va;this.draw=function(a){k.isAnimating?
enableConsoleDebugging&&console.log("ANIMATING: Skipping draw:"+a):(Va=a,!0===Ta?!0!==Ua&&(Ua=!0,ob.init(10,function(){Ua=Ta=!1;k._draw(Va)})):(Ta=!0,k._draw(Va)))};this._draw=function(a){enableConsoleDebugging&&console.log("draw:"+a);enableConsoleDebugging&&console.log("drawnBBOX.setBBOX(bbox)");O.setBBOX(p);a:{B.triggerEvent("beforedraw");if(!1===k.isAnimating&&void 0!==a&&"object"===typeof a&&0<a.length){if(a.length>maxAnimationSteps){alert("Too many animations given, max is "+maxAnimationSteps);
k.draw("self");break a}k.isAnimating=!0;B.triggerEvent("onstartanimation",k);k.currentAnimationStep=0;k.animationList=[];k.mouseHoverAnimationBox=!1;for(var b=0;b<a.length;b++){var c={name:a[b].name,value:a[b].value};k.setDimension(a[b].name,a[b].value,!1);c.requests=k.getWMSRequests();k.animationList.push(c)}k.setDimension(k.animationList[k.currentAnimationStep].name,k.animationList[k.currentAnimationStep].value,!1);db.checkAnimation()}k._pdraw()}};var pb=function(){debug("--\x3e onmapready event called");
...
if(a.isLoading())return 1};this.mouseHoverAnimationBox=!1;this._pdraw=function(){if(0!=ab)if(fa)1==B.addToCallback("onmapready",pb)&&debug("Suspending on onmapready");else{enableConsoleDebugging&&console.log("loadLayers");var a,b=0;T=0;if(F)for(var c=0;c<F.length;c++)F[c].enabled&&!1===F[c].keepOnTop&&(!F[c].type||"twms"!==F[c].type)&&(T++,a=e(F[c]))&&(J[da].setSrc(b,a,k.getWidth(),k.getHeight()),J[da].setOpacity(b,F[c].opacity),b++);for(c=c=0;c<k.getNumLayers();c++)t[c].service&&t[c].enabled&&(a=
e(t[c],t[c].dimensions))&&(J[da].setSrc(b,a),J[da].setOpacity(b,t[c].opacity),t[c].image=J[da].layers[b],b++);if(F)for(c=0;c<F.length;c++)F[c].enabled&&1==F[c].keepOnTop&&(a=e(F[c]))&&(J[da].setSrc(b,a),J[da].setOpacity(b,F[c].opacity),b++);k.flipBuffers();B.triggerEvent("ondrawready",k);ja()}};var db,la=function(a){if(0!=J.length){var b=p;isDefined(a)&&(b=a);D.copy(b);J[ia].setBBOX(D,ea);J[ia].mapbbox=D;k.showBoundingBox(A.bbox,D);B.triggerEvent("onupdatebbox",D)}},eb=new WMJSTimer;k.flipBuffers=
function(){enableConsoleDebugging&&console.log("flipBuffers");var a=ia,b=da;B.triggerEvent("onmapstartloading");fa=!0;eb.init(500,function(){Ka.show()});pa.setBBOX(p);J[b]&&J[b].load(function(){enableConsoleDebugging&&console.log("flipBuffers loadcomplete");try{J[a].srs=q,J[b].bbox=p.clone(),J[b].srs=q,enableConsoleDebugging&&console.log("loadedBBOX.setBBOX(bbox)"),ea.setBBOX(pa),enableConsoleDebugging&&console.log("-----------------------"),J[b].display(D,ea),w.oldx=w.exactX,w.oldy=w.exactY,J[a].hide(),
ia=b,da=a}catch(c){console.log(c)}fa=!1;B.triggerEvent("onmaploadingcomplete",k);B.triggerEvent("onloadingcomplete",k);B.triggerEvent("onmapready",k);Ka.hide();eb.stop()})};k.getBackBufferCanvasContext=function(){return J[da].getCanvasContext()};k.getFrontBufferCanvasContext=function(){return J[ia].getCanvasContext()};k.redrawBuffer=function(){J[ia].display()};this.addBaseLayers=function(a){if(a){T=0;a=toArray(a);F.push(a);for(j=0;j<F.length;j++)1==F.keepOnTop&&T++;B.triggerEvent("onlayeradd")}else F=
void 0};this.setBaseLayers=function(a){if(a){T=0;F=a;for(j=0;j<F.length;j++)1==F.keepOnTop&&T++;B.triggerEvent("onlayerchange")}else F=void 0};this.getBaseLayers=function(){return F};this.getNumLayers=function(){return t.length};this.getBaseElement=function(){return n};var ua=0;new WMJSTimer;var va=0,qb=new WMJSTimer,ha=0,wa=new WMJSBBOX,xa=new WMJSBBOX,fb=new WMJSBBOX,ya=0,Ea=0,hb=function(){var a=1-ha,a=new WMJSBBOX(wa.left*a+xa.left*ha,wa.bottom*a+xa.bottom*ha,wa.right*a+xa.right*ha,wa.top*a+xa.top*
ha);la(a);va+=1;ha+=1/6;5<va?(ha=va=0,k.zoomTo(a),0===Ea?(ya=Ea=0,k.draw("flyZoomToBBOXTimerFunc")):(ya=Ea=0,gb(D,fb))):qb.init(10,hb)},gb=function(a,b){1===ya?(fb.copy(b),Ea=1):(wa.copy(a),xa.copy(b),ha=va=0,0===ya&&(ya=1,hb()))},ib=new WMJSBBOX,za=new WMJSBBOX;this.mouseWheelEvent=function(a,b,c,e){a.stopPropagation();preventdefault_event(a);if(1!=ua){ua=1;var d=D.right-D.left,f=D.bottom-D.top;a=k.getGeoCoordFromPixelCoord({x:H,y:I},O);e=(a.x-D.left)/d;c=(a.y-D.top)/f;var g;0>b?(d*=-.3,g=-.3*f):
(d*=.2,g=.2*f);b=D.left+d;f=D.top+g;d=D.right-d;g=D.bottom-g;e=e*(d-b)+b-a.x;a=c*(g-f)+f-a.y;b-=e;d-=e;f-=a;g-=a;ib.copy(D);za.left=b;za.bottom=g;za.right=d;za.top=f;gb(ib,za);ua=0}};var N;this.pinchStart=function(a,c,e){b=e.pointers[0].clientX;d=e.pointers[0].clientY;f=e.pointers[1].clientX;g=e.pointers[1].clientY;N=p.clone();W=0;ua=aa=1};this.pinchMove=function(a,c,e){W=0;pinchMove1={x:e.pointers[0].clientX,y:e.pointers[0].clientY};pinchMove2={x:e.pointers[1].clientX,y:e.pointers[1].clientY};a=
...
try{e+="&"+qa(a)}catch(f){return}debug("<a target='_blank' href='"+e+"'>"+e+"</a>",!1);return e};var Fa=function(a,b){ka--;var c;c=b?1==b.queryable?new Xa(b,a):new Xa(b,"not queryable"):new Xa(b,"Query failed...");Wa.push(c);0>=ka&&(ka=0,B.triggerEvent("ongetfeatureinfoready",Wa))},sb=function(){debug("resuming on ongetpointinfoready");k.getPointInfo(L,M)},Ga=!1;this.getPointInfo=function(a,b){if(Ga)debug("suspending on ongetpointinfoready"),k.addListener("ongetpointinfoready",sb,!1);else{Ga=!0;B.triggerEvent("beforegetpointinfo");
var c=k.getGraphingData();if(void 0==c)error("getPointInfo getGraphingData is undefined"),Ga=!1;else{debug("getPointInfo("+a+","+b+") "+c.layer.name);var e=rb(c.layer,a,b,c.service,c.style);debug("GetPointInfo: "+e);var d=new Image;d.loadEvent=function(){Ga=!1;var a={};a.url=e;a.layer=c.layer;a.img=d;B.triggerEvent("ongetpointinfoready",a)};l(d);d.src=e}}};var tb=function(){debug("resuming on ongetfeatureinfoready");B.triggerEvent("beforegetfeatureinfo");k.getFeatureInfo(L,M)};this.getFeatureInfo=
function(a,b){if(0<ka)debug("suspending on ongetfeatureinfoready"),k.addListener("ongetfeatureinfoready",tb,!1);else{debug("GetFeatureInfo:");Wa=[];for(var c=ka=0;c<t.length;c++){var e=t[t.length-c-1];e.getFeatureInfoUrl="";e.service&&e.enabled&&1==e.queryable&&(e.getFeatureInfoUrl=k.getWMSGetFeatureInfoRequestURL(e,a,b),isDefined(e.getFeatureInfoUrl)?ka++:e.getFeatureInfoUrl="")}0==ka&&B.triggerEvent("ongetfeatureinfoready",["No layers to query"]);for(c=0;c<t.length;c++)if(e=t[t.length-c-1],""!=
...
{map:k,x:L,y:M}),k.getFeatureInfo(L,M)):"point"==P&&(k.setMapPin(L,M),k.showMapPin(),k.getPointInfo(L,M))};var jb=!1,R=!1,Ha,Ya;this.updateMouseCursorCoordinates=function(a){Ya=a;Ha=k.getGeoCoordFromPixelCoord(a);k.draw("updateMouseCursorCoordinates")};this.mouseDownEvent=function(a){preventdefault_event(a);var b=k.getMouseCoordinatesForDocument(a);oa&&30>b.y||k.mouseDown(b.x,b.y,a)};this.mouseMoveEvent=function(a){preventdefault_event(a);var b=k.getMouseCoordinatesForDocument(a);0===W&&0<=b.y&&30>
b.y&&0<=b.x&&b.x<=z?!1===oa&&(oa=!0,Za=Ja,Ba=!0,k.setCursor("pointer"),k.draw("mouseMoveEvent")):!0===oa&&(Ba=oa=!1,k.setCursor(Za),k.draw("mouseMoveEvent"));k.mouseMove(b.x,b.y,a)};this.mouseUpEvent=function(a){preventdefault_event(a);var b=k.getMouseCoordinatesForDocument(a);k.mouseUp(b.x,b.y,a)};this.mouseMove=function(b,c){H=b;I=c;if(0===aa)for(var e=B.triggerEvent("beforemousemove",{mouseX:H,mouseY:I,mouseDown:1===W?!0:!1}),d=0;d<e.length;d++)if(!1===e[d])return;if(1==A.displayed&&0==ma){var e=
k.getPixelCoordFromGeoCoord({x:A.bbox.left,y:A.bbox.top}),d=k.getPixelCoordFromGeoCoord({x:A.bbox.right,y:A.bbox.bottom}),f=!1;0==W&&(!1===R&&(jb=n.css("cursor")),6>Math.abs(H-e.x)&&I>e.y&&I<d.y&&(f=!0,n.css("cursor","col-resize"),R="left"),6>Math.abs(I-e.y)&&H>e.x&&H<d.x&&(f=!0,n.css("cursor","row-resize"),R="top"),6>Math.abs(H-d.x)&&I>e.y&&I<d.y&&(f=!0,n.css("cursor","col-resize"),R="right"),6>Math.abs(I-d.y)&&H>e.x&&H<d.x&&(f=!0,n.css("cursor","row-resize"),R="bottom"),6>Math.abs(H-e.x)&&6>Math.abs(I-
...
H,d.y=I),"bottomright"==R&&(d.x=H,d.y=I),e=k.getGeoCoordFromPixelCoord(e),d=k.getGeoCoordFromPixelCoord(d),A.bbox.left=e.x,A.bbox.top=e.y,A.bbox.right=d.x,A.bbox.bottom=d.y,k.showBoundingBox(A.bbox),B.triggerEvent("bboxchanged",{map:k,bbox:A.bbox}));return}R=!1;n.css("cursor",jb)}if(-1==(0>I|0>H|H>z|I>C?-1:0)){try{B.triggerEvent("onmousemove",[void 0,void 0]),k.updateMouseCursorCoordinates(void 0)}catch(g){console.log(g)}Y=H;Z=I;0!=ma&&(1==W&&"zoomout"==P&&k.zoomOut(),W=0,1==aa&&kb(Y,Z))}else{if(1==
...
lb(f,m):(m=k.getGeoCoordFromPixelCoord({x:f,y:m},O),f=m.x-na.x,m=m.y-na.y,k.setMapPin(w.oldx+f/(p.right-p.left)*z,w.oldy+m/(p.bottom-p.top)*C),D.left=O.left-f,D.bottom=O.bottom-m,D.right=O.right-f,D.top=O.top-m,la(D)));"zoom"==P&&0!=Ia&&(d=H-L,e=I-M,0>d&&0>e?n.css("cursor","not-allowed"):n.css("cursor","crosshair"),h=e,G.style.display="",0>d?(d=-d,G.style.left=H+"px"):G.style.left=L+"px",0>h?(h=-h,G.style.top=I+"px"):G.style.top=M+"px",G.style.width=d+"px",G.style.height=h+"px")}B.triggerEvent("onmousemove",
[H,I]);k.updateMouseCursorCoordinates({x:H,y:I})}};this.mouseUp=function(a,b,e){Y=a;Z=b;if(0===aa&&0===(0>Z|0>Y|Y>z|Z>C?-1:0))for(a=B.triggerEvent("beforemouseup",{mouseX:a,mouseY:b,mouseDown:!1,event:e}),b=0;b<a.length;b++)if(!1===a[b]){W=0;return}if(1==W){"zoomout"==P&&k.zoomOut();if(0==aa&&3>Math.abs(L-Y)&&3>Math.abs(M-Z)&&(isDefined(e)&&B.triggerEvent("mouseclicked",{map:k,x:Y,y:Z,shiftKeyPressed:1==e.shiftKey}),1==bb)){var c;0==U.length?(c=WMJSDialog.createDialog({x:Y,y:Z,autoDestroy:!1},n,k),
U.push(c)):c=U[0];0==c.hasBeenDragged&&(1==c.moveToMouseCursor?c.setXY(Y,Z):c.setXY(5,35));c.on("hide",function(a,b){c.hasBeenDragged=!1});c.setLoading();c.show();k.addListener("ongetfeatureinfoready",function(a){c.setHTML(k.getGetFeatureInfoObjectAsHTML(a))},!0);k.setMapPin(L,M);k.showMapPin();B.triggerEvent("beforegetfeatureinfo");k.getFeatureInfo(L,M)}B.triggerEvent("mouseup",{map:k,x:Y,y:Z})}W=0;1==aa&&kb(Y,Z)};var kb=function(a,b){if(0!=aa){aa=0;"pan"==P&&lb(a,b);if("zoom"==P){var c=a,e=b,c=
Y-L,e=Z-M;n.css("cursor","default");if(0!=Ia&&(Ia=0,G.style.display="none",!(0>c&&0>e))){var d=new WMJSBBOX;0>c?(d.left=L+c,d.right=L):(d.left=L,d.right=L+c);0>e?(d.top=M+e,d.bottom=M):(d.top=M,d.bottom=M+e);c=k.pixelCoordinatesToXY({x:d.left,y:d.bottom});e=k.pixelCoordinatesToXY({x:d.right,y:d.top});d.left=c.x;d.bottom=c.y;d.right=e.x;d.top=e.y;k.zoomTo(d);k.draw("mapZoomEnd")}}B.triggerEvent("mapdragend",{map:k,x:Y,y:Z})}},ma=0,na,lb=function(a,b){n.css("cursor","default");var c=parseInt(a),e=parseInt(b);
...
a.style.height);for(b=0;2>b;b++){var c=new WMJSCanvasBuffer(B,"imagebuffer",getMapImageStore,k.getWidth(),k.getHeight());getMapImageStore.addLoadEventCallback(c.imageLoadComplete);n.append(c.getBuffer());J.push(c)}for(b=0;2>b;b++)c=new WMJSCanvasBuffer(B,"legendbuffer",legendImageStore,k.getWidth(),k.getHeight()),legendImageStore.addLoadEventCallback(c.imageLoadComplete),n.append(c.getBuffer()),S.push(c);B.addToCallback("draw",k.draw,!0);db=new WMJSAnimate(k);bgMapImageStore.addLoadEventCallback(k.draw);
...
a.rect(0,0,z,30);!1===$a?(a.globalAlpha=Ba?.9:.4,a.fillStyle=Ba?"#017daf":"#EEE"):(a.globalAlpha=1,a.fillStyle="#017daf");a.fill();a.globalAlpha=1;""!==Pa&&(a.font="20px Helvetica",e(a,Pa,z/2-70,C-26,20));""!==Qa&&(a.font="15px Helvetica",e(a,Qa,z/2-70,2,15));if(!0===Sa){var b=function(){var a=0,b=1E-7,c=z/(D.right-D.left);if(!(0>=c)){var e=25/c,a=0;do{b*=10;a=e/b;if(0===a)return;a=25/a}while(25>a);do{b/=2;a=e/b;if(0===a)return;a=25/a}while(25<a);do{b*=1.2;a=e/b;if(0===a)return;a=25/a}while(25>a);
...
isDefined(Ya)&&(b=k.getLatLongFromPixelCoord(Ya),isDefined(b)&&(a.fillStyle="#000000",c=Math.round(100*b.x)/100,a.fillText("Lat/Lon: ("+(Math.round(100*b.y)/100).toFixed(2)+", "+c.toFixed(2)+")  degrees",5,C-38)));a.fillStyle="#000000";a.fillText("Map projection: "+q,5,C-26)},!0);ab=1})();la(p)};WMJSEmptyLayerName="empty_layer";WMJSEmptyLayerTitle="empty layer";
