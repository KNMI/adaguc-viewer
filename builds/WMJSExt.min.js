...
resize:{fn:function(){b.applicationSettings.app&&b.applicationSettings.app.resize(b.width,mainWebmapJS.webMapJS.getHeight())}}},setApplication:function(g){b.applicationSettings=g}});a=Ext.create("Ext.panel.Panel",{region:"center",layout:"border",split:!0,frame:!1,border:!1,margin:0,items:[mainWebmapJS,b]});var c=function(g){var k=createNewLayerPanel(mainWebmapJS.webMapJS,{service:g.service,name:g.layer,style:g.style,opacity:g.opacity,ready:function(v,t){var n=g.srs,r=g.bbox,w=g.baselayername,x=g.baselayerservice;
n&&!r?t.setProjection(v.getProjection(n)):n&&r&&t.setProjection({srs:n,bbox:r});w&&x?(mainWebmapJS.baseLayer.setService(x),mainWebmapJS.baseLayer.name=w,mainWebmapJS.baseLayer.parseLayer(function(){t.draw("dataPanelClicked::parseLayer")},void 0,"WMJSExtMain::dataPanelClicked")):t.draw("dataPanelClicked");isDefined(k)&&isDefined(g.ready)&&(g.ready(k,t),1==k.WMJSLayer.hasError&&k.close())}});return k},e,f=new Console;Ext.create("Ext.panel.Panel",{html:'<div id="debuginfodiv"></div>',layout:"fit",listeners:{afterrender:{fn:function(){f.setElement(document.getElementById("debuginfodiv"))}}}});
var p=[{text:I18n.show_time_selection_window.text,iconCls:"button_settings_icon",handler:function(){timeselectorWindow.show()}},{text:I18n.create_animation.text+"&hellip;",iconCls:"button_settings_icon",handler:function(){Ext.create("Ext.window.Window",{title:I18n.create_animation.text,width:460,height:300,layout:"form",autoDestroy:!0,closeAction:"destroy",autoScroll:!0,collapsible:!0,items:[Ext.create("webmapjsext.WMJSExt.AnimationPanel",{mappanel:mainWebmapJS})]}).show()}},{text:I18n.create_link.text,
...
{text:I18n.undo_zoom_pan_action.text,iconCls:"button_undo",handler:function(){mainWebmapJS.webMapJS.setPreviousExtent()}},{text:I18n.redo_zoom_pan_action.text,iconCls:"button_redo",handler:function(){mainWebmapJS.webMapJS.setNextExtent()}},{text:I18n.abort_loading.text,iconCls:"button_stoploading",handler:function(){mainWebmapJS.webMapJS.abort()}}];try{isDefined(getFeatureInfoApplications)}catch(g){getFeatureInfoApplications=[]}for(var l={},h=0;h<getFeatureInfoApplications.length;l={$jscomp$loop$prop$location$9$16:l.$jscomp$loop$prop$location$9$16},
h++)l.$jscomp$loop$prop$location$9$16=getFeatureInfoApplications[h],p.push({text:getFeatureInfoApplications[h].name,iconCls:getFeatureInfoApplications[h].iconCls,location:l.$jscomp$loop$prop$location$9$16,handler:function(g){return function(){b.setApplication(g.$jscomp$loop$prop$location$9$16);b.hide();b.show()}}(l)}),getFeatureInfoApplications.open&&l.$jscomp$loop$prop$location$9$16.name===getFeatureInfoApplications.open&&b.setApplication(l.$jscomp$loop$prop$location$9$16);p.push({text:I18n.about_the_adaguc_viewer.text,
iconCls:"button_info",tooltip:I18n.about_the_adaguc_viewer.tooltip,handler:function(){Ext.create("Ext.window.Window",{title:I18n.about_adaguc.text,height:200,width:400,modal:!0,layout:"fit",items:{xtype:"panel",html:'<div class="about_adaguc" style="position:absolute;top:0px;left:0px;height:100px;width:520px;"/><div style="position:absolute;top:100px;padding:10px;margin:10px;">'+versionInfo+I18n.about_adaguc_more_information.text}}).show()}});l=Ext.create("Ext.panel.Panel",{header:!1,title:" ",frame:!1,
...
g)}catch(k){}}}}}),q=function(g){var k=!1,v=!1;isDefined(g.collapse)&&"1"==g.collapse&&Ext.getCmp("viewportwestpanel").collapse();isDefined(g.zoomtolayer)&&"1"==g.zoomtolayer&&(k=!0);isDefined(g.srs)&&isDefined(g.bbox)&&mainWebmapJS.webMapJS.setProjection({srs:g.srs,bbox:g.bbox});if(isDefined(g.service)){clearLayers(mainWebmapJS.webMapJS);var t=URLDecode(g.service).split(","),n=[""];isDefined(g.layer)?n=URLDecode(g.layer).split(","):1==k&&(v=!0);isDefined(g.selected);for(var r=n.length,w=0;w<n.length;w++)try{var x=
n[n.length-1-w].split("$"),y=x+"",A=!0,z="",C=1,B=t[0]+"";6==x.length&&(y=x[0],A=x[2],z=x[3],C=x[4],isDefined(A)&&("true"===A&&(A=!0),"false"===A&&(A=!1)),B=t[x[5]]);createNewLayerPanel(mainWebmapJS.webMapJS,{service:B,name:y,style:z,enabled:A,opacity:C,ready:function(D,E){r--;if(0==r){if(isDefined(g.dims))for(var H=g.dims.split(","),F=0;F<H.length;F++){var G=H[F].split("$");2==G.length&&mainWebmapJS.webMapJS.setDimension(G[0],G[1])}1==k&&D.WMJSLayer.zoomToLayer();E.draw("index.html::numlayers=0")}},
autoChooseLayer:v})}catch(D){error("Unable to parse layer nr "+w)}if(isDefined(g.baselayers)){t=g.baselayers.split("$");v=[];for(w=n=0;w<t.length;w++){x=!1;for(y=0;y<mapTypeConfiguration.length;y++)if(mapTypeConfiguration[y].baselayer.name==t[w]){mapTypeConfiguration[y].baselayer.enabled=!0;v.push(mapTypeConfiguration[y].baselayer);x=!0;n++;break}if(!x)for(y=0;y<baseLayerConfiguration.length;y++)if(baseLayerConfiguration[y].name==t[w]){baseLayerConfiguration[y].enabled=!0;v.push(baseLayerConfiguration[y]);
x=!0;n++;break}x||error("Warning: unable to find baselayer "+t[w])}for(w=0;w<baseLayerConfiguration.length;w++)0==baseLayerConfiguration[w].enabled?v.push(baseLayerConfiguration[w]):0==n&&v.push(baseLayerConfiguration[w]);baseLayers=[];for(w=0;w<v.length;w++)t=new WMJSLayer(v[w]),0==w&&(mainWebmapJS.baseLayer=t),baseLayers.push(t);0<baseLayers.length&&mainWebmapJS.webMapJS.setBaseLayers(baseLayers)}}};0==window.location.hash.length&&q(getUrlVars());checkIfHashTagChanged(function(g,k){if(0<=g.indexOf("addlayer('")){if(k=
g.substring(g.indexOf("addlayer('")+10,g.indexOf("',")),g=g.substring(g.indexOf(",'")+2,g.indexOf("')")),k&&g){for(var v=!1,t=mainWebmapJS.webMapJS.getLayers(),n=0;n<t.length;n++)if(t[n].service==k&&t[n].name==g){v=!0;break}0==v&&(isDefined(hashLocationNotfiyAddLayer),firstTimeOpening=!0,createNewLayerPanel(mainWebmapJS.webMapJS,{service:k,name:g,zoomtolayer:!1,notifyadded:!0}))}}else 0<=g.indexOf("positionmappinbylatlon(")?(debug(g),k=g.substring(23,g.indexOf(")")),debug(k),k=k.split(","),debug(k[0]+
" "+k[1]),mainWebmapJS.webMapJS.positionMapPinByLatLon({x:parseFloat(k[0]),y:parseFloat(k[1])})):0<=g.indexOf("refreshlayers")?updateLayers():q(k)});getFeatureInfoApplications.open&&b.show()});var gfiapp_d3c3=function(a,b){var c=function(q,g){$("#"+g).empty();$("#"+g).append("<div style='height=240px;border=2px solid blue;' class='chart'>a</div>");$("#"+g).append("<div style='height=40px;border=2px solid red;' class='controls gfiapp_d3c3_export_to_csv_button'>b</div>");var k=$("#"+g).find(".chart");g=$("#"+g).find(".controls");k.html("graph");g.button({label:"Export to CSV"}).click(function(){console.log("Export to CSV");for(var v="",t=q.datatoplot,n=0;n<t[0].length;n++){for(var r=0;r<
