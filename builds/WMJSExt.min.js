...
try{x=formatDescriptionLookup[w]}catch(y){}isDefined(x)||(x=n.formats[r]);h.add({title:x,format:n.formats[r]});"AAIGRID"==w&&m.getView().select(r)}t();v.update('<table style="margin:5px;"><tr><td><b>Coverage:</b></td><td>'+n.name+"</td></tr><tr><td><b>Title:</b></td><td>"+n.title+"</td></tr><tr><td><b>Description:</b></td><td>"+n["abstract"]+"</td></tr></table>")};Ext.create("Ext.panel.Panel",{region:"west",border:!0,width:100});var u=Ext.create("Ext.panel.Panel",{region:"center",border:!1,layout:{type:"hbox",
...
{text:I18n.undo_zoom_pan_action.text,iconCls:"button_undo",handler:function(){mainWebmapJS.webMapJS.setPreviousExtent()}},{text:I18n.redo_zoom_pan_action.text,iconCls:"button_redo",handler:function(){mainWebmapJS.webMapJS.setNextExtent()}},{text:I18n.abort_loading.text,iconCls:"button_stoploading",handler:function(){mainWebmapJS.webMapJS.abort()}}];try{isDefined(getFeatureInfoApplications)}catch(g){getFeatureInfoApplications=[]}for(var l={},h=0;h<getFeatureInfoApplications.length;l={$jscomp$loop$prop$location$9$17:l.$jscomp$loop$prop$location$9$17},
h++)l.$jscomp$loop$prop$location$9$17=getFeatureInfoApplications[h],p.push({text:getFeatureInfoApplications[h].name,iconCls:getFeatureInfoApplications[h].iconCls,location:l.$jscomp$loop$prop$location$9$17,handler:function(g){return function(){b.setApplication(g.$jscomp$loop$prop$location$9$17);b.hide();b.show()}}(l)}),getFeatureInfoApplications.open&&l.$jscomp$loop$prop$location$9$17.name===getFeatureInfoApplications.open&&b.setApplication(l.$jscomp$loop$prop$location$9$17);p.push({text:I18n.about_the_adaguc_viewer.text,
...
g)}catch(k){}}}}}),q=function(g){var k=!1,v=!1;isDefined(g.collapse)&&"1"==g.collapse&&Ext.getCmp("viewportwestpanel").collapse();isDefined(g.zoomtolayer)&&"1"==g.zoomtolayer&&(k=!0);isDefined(g.srs)&&isDefined(g.bbox)&&mainWebmapJS.webMapJS.setProjection({srs:g.srs,bbox:g.bbox});if(isDefined(g.service)){clearLayers(mainWebmapJS.webMapJS);var t=URLDecode(g.service).split(","),n=[""];isDefined(g.layer)?n=URLDecode(g.layer).split(","):1==k&&(v=!0);isDefined(g.selected);for(var r=n.length,x=0;x<n.length;x++)try{var w=
...
!0);b.enableInlineGetFeatureInfo(!1)};this.enable=function(p,l){f(p,l)};this.disable=function(){b.enableInlineGetFeatureInfo(!0)};this.resize=function(p,l){for(var h=0;h<e.length;h++)e[h].setSize(p,l/e.length);for(h=0;h<e.length;h++);};c.enable()};try{WMJSExtApplications.EProfile=gfiapp_eprofile}catch(a){};var autowms_app=function(a,b){initWMJS();a.addClass("autowms_app_container");var c="https://geoservices.knmi.nl/autowms?";try{c=autowmsURL}catch(g){}var e={},f=[],p=function(g){var k=m();a.html(k+JSON.stringify(g));$(".autowms_app_fileitem_return").attr("onclick","").click(function(v){f=[];q("")});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(v){var t=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){q(t)},100)})},
...
(k+'">&#8635;&nbsp;<b>Refresh</b></span></span>')},u=function(g,k){a.html("... working ... ");var v=WMJSGetServiceFromStore(k,xml2jsonrequestURL);v.getLayerObjectsFlat(function(t){var n=""+m(g,k);n+="<span class='autowms_app_serviceabstract'><b>"+v.title.replace(/!/g,"<br/>")+"</b><hr/>"+v.abstract.replace(/!/g,"<br/>");if(k&&(n+='<hr/>WMS: <a target="_blank" href="'+k+'&service=WMS&request=GetCapabilities">'+k+"</a>",1<k.split("?").length)){var r=new WMJSKVP(k.split("?")[1]),x=k.split("?")[0].replace("adagucserver",
"adagucopendap/");x=x.replace(/([^:]\/)\/+/g,"$1");r.source&&(x+=r.source);r.dataset&&(x+=r.dataset+"/");n=n+('<hr/>OpenDAP: <span class=\'autowms_app_opendaplinks\'><a target="_blank" href="'+x+'">'+x+'</a><a target="_blank" href="')+(x+'.das">DAS</a><a target="_blank" href="')+(x+'.dds">DDS</a></span>')}n+="</span>";a.html("... working ... ");e={};for(r=0;r<t.length;r++)n+=h(t[r],k),e[t[r].name]=t[r];a.html(n);$(".autowms_app_layeritem").attr("onclick","").click(function(w){w=e[$(this).attr("name")];
window.location.hash="addlayer('"+k+"','"+w.name+"')"});$(".autowms_app_fileitem_return").attr("onclick","").click(function(w){q(f.pop())});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(w){var y=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){u(g,y)},100)});$(".autowms_app_request_button").attr("onclick","").click(function(w){q("")});$(".autowms_app_request_input").keyup(function(w){13==w.keyCode&&q("")})},
...
var v=function(n){if(n.error)a.html("Error from server: "+n.error);else{e={};f[f.length-1]!==g&&f.push(g);var r=m(n,g);a.html("... working ... ");n.result.sort(function(w,y){return w.name.localeCompare(y.name)});n.result.sort(function(w,y){if(w.leaf===y.leaf)return 0;if(!0===w.leaf)return 1;if(!0===y.leaf)return-1});for(var x=0;x<n.result.length;x++)r+=l(n.result[x]),e[n.result[x].name]=n.result[x];a.html(r);$(".autowms_app_fileitem_return").attr("onclick","").click(function(w){f.pop();q(f[f.length-
1])});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(w){var y=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){q(y)},100)});$(".autowms_app_fileitem").attr("onclick","").click(function(w){w=e[$(this).attr("name")];!1===w.leaf?q(w.path):u(n,w.adaguc)});$(".autowms_app_request_button").attr("onclick","").click(function(w){q("")});$(".autowms_app_request_input").keyup(function(w){13==w.keyCode&&q("")})}},t=c+"request=getfiles&path="+
g;a.html('... Reading <a href="'+t+'">'+t+"</a> ... ");try{$.ajax({contentType:"application/json",crossDomain:!0,type:"GET",url:t,success:v,error:function(n){console.error("Request failed without jsonp, trying jsonp");v({result:[]});$.ajax({dataType:"jsonp",contentType:"application/jsonp",crossDomain:!0,type:"GET",url:t,success:v,error:function(r){p("Unable to do ajax call")}})}})}catch(n){console.error(n),p(n)}};this.enable=function(){a.html("<div>Initializing...</div>");q("")};this.disable=function(){};
this.resize=function(g,k){}};try{WMJSExtApplications.AutoWMS=autowms_app}catch(a){};
