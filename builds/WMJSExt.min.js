...
95;c.timer.init(100,c.fadeOut)})}},show:{fn:function(){c.getEl().dom.style.opacity=1;c.opacity=100}}}});this.callParent(arguments)}});initWMJS();var error=function(c){console.error(c)},debug=function(c){console.log(c)},versionInfo="ADAGUC viewer version AEMET 1.2.4",mainWebmapJS;function decimalFormatter(c,n){c=parseInt(c)+"";n=parseInt(n)-c.length;var x,y="";for(x=0;x<n;x++)y+="0";return y+c}
...
var ba=(ff-R)/36E5;G=Object.keys(O).toString()}var ia=c.includes("ECMWF")||c.includes("HARMONIE")?T.toString():G.toString();ia=ia.split("T");var la=ia[0].replaceAll("-","");la=parseInt(la);var ma=parseInt(ia[1].substring(0,2));T="OBSERVACION";if(c.includes("KRR")||c.includes("KRF")||c.includes("ASV_FCT")){ma=ia[1].substring(0,5);var ua="IASI-KRR/F";var Da="";T="SAT-PROFILE"}c.includes("ECMWF")&&(ua="ECMWF",Da="",T="MODEL");c.includes("HARMONIE")&&(ua="HARMONIE",Da="",T="MODEL");for(ia=0;ia<w.length;ia++){console.log(w[ia].standard_name);
if("station_name_backup"==w[ia].name){if(isDefined(ua))continue;ua=w[ia].data[G]}if("ps"==w[ia].name){if(isDefined(va))continue;var va=w[ia].data[G]/100;isNaN(va)&&(va=w[ia].data[0]/100)}if("air_pressure_at_mean_sea_level"==w[ia].standard_name||"medium_sea_level_pressure"==w[ia].standard_name){if(isDefined(va))continue;va=parseFloat(O[G]);isNaN(va)&&(va=parseFloat(O[G]))}if("surface_air_pressure"==w[ia].name){if(isDefined(va))continue;va=w[ia].data[G]/100;isNaN(va)&&(va=w[ia].data[0]/100)}if("surface_air_temperature"==
w[ia].standard_name){if(isDefined(Fa))continue;var Fa=isDefined(w[ia].data[2])?w[ia].data[2][G]-273.15:w[ia].data[G]-273.15;isNaN(Fa)&&(Fa=w[ia].data[2][G]-273.15)}if("surface_dew_point_temperature"==w[ia].standard_name){if(isDefined(Ma))continue;var Ma=isDefined(w[ia].data[2])?w[ia].data[2][G]-273.15:w[ia].data[G]-273.15;isNaN(Ma)&&(Ma=w[ia].data[2][G]-273.15)}if("alt"==w[ia].name){if(isDefined(Ka))continue;var Ka=parseFloat(w[ia].data[G]);isNaN(Ka)&&(Ka=parseFloat(w[ia].data[0]))}"station_cname_backup"!=
w[ia].name||isDefined(Da)||(Da=w[ia].data[G])}if(isDefined(ua)&&"nodata"!=ua){isNaN(Ka)&&(Ka=0);x={model:T,index:ua,date:la,run:ma,step:0,lon:L,lat:P,ps:va,zs:Ka,name:ua+" "+Da,ts:Fa,tds:Ma};if(c.includes("ECMWF")||c.includes("HARMONIE"))x={model:"ECMWF",index:"MODEL",date:la,run:ma,step:ba,lon:L,lat:P,ps:va,zs:0,name:"MODEL"};n(x)}else n(null)}catch(Va){n(null)}}})}
