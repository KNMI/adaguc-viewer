...
"&service=WMS&request=getmap&format=image/png&layers="+Z.name+"&width=400&CRS=EPSG:4326&STYLES=&EXCEPTIONS=INIMAGE&showlegend=true&dim_reference_time=*&"+Math.random())+'" /></span></span>'},aa=function(Z,ma){return"<span class='autowms_app_currentpath'>AutoWMS link: <span class='autowms_app_request_container'><input class='autowms_app_request_input' type=\"text\" value=\""+z+"\"></input>  <span class='autowms_app_request_button'>-></span></span><hr/><span class='autowms_app_currentpath'>Current folder: ./"+
(ma+'</span><span class="autowms_app_fileitem_header"><span class="autowms_app_fileitem_return">&#128193;&nbsp;<b>../ (&#8679;)</b></span><span class="autowms_app_fileitem_refresh" name="')+(ma+'">&#8635;&nbsp;<b>Refresh</b></span></span>')},ca=function(Z,ma){e.html("... working ... ");var za=WMJSGetServiceFromStore(ma,xml2jsonrequestURL);za.getLayerObjectsFlat(function(va){var ua=""+aa(Z,ma);ua+="<span class='autowms_app_serviceabstract'><b>"+za.title.replace(/!/g,"<br/>")+"</b><hr/>"+za.abstract.replace(/!/g,
"<br/>");if(ma&&(ua+='<hr/>WMS: <a target="_blank" href="'+ma+'&service=WMS&request=GetCapabilities">'+ma+"</a>",1<ma.split("?").length)){var Ba=new WMJSKVP(ma.split("?")[1]),Na=ma.split("?")[0].replace("adagucserver","adagucopendap/");Na=Na.replace(/([^:]\/)\/+/g,"$1");Ba.source&&(Na+=Ba.source);Ba.dataset&&(Na+=Ba.dataset+"/");ua=ua+('<hr/>OpenDAP: <span class=\'autowms_app_opendaplinks\'><a target="_blank" href="'+Na+'">'+Na+'</a><a target="_blank" href="')+(Na+'.das">DAS</a><a target="_blank" href="')+
(Na+'.dds">DDS</a></span>')}ua+="</span>";e.html("... working ... ");F={};for(Ba=0;Ba<va.length;Ba++)ua+=V(va[Ba],ma),F[va[Ba].name]=va[Ba];e.html(ua);$(".autowms_app_layeritem").attr("onclick","").click(function(La){La=F[$(this).attr("name")];window.location.hash="addlayer('"+ma+"','"+La.name+"')"});$(".autowms_app_fileitem_return").attr("onclick","").click(function(La){ea(I.pop())});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(La){var Oa=$(this).attr("name");$(".autowms_app_fileitem").remove();
$(".autowms_app_layeritem").remove();setTimeout(function(){ca(Z,Oa)},100)});$(".autowms_app_request_button").attr("onclick","").click(function(La){ea("")});$(".autowms_app_request_input").keyup(function(La){13==La.keyCode&&ea("")})},function(va){va=aa();va=va+"<span class='autowms_app_error_container'><span class='autowms_app_error_container_header'>Unable to get WMS GetCapabilities</span><a target='_blank' href=\""+(ma+'service=WMS&request=GetCapabilities">'+ma+"</a>");e.html(va+"</span>");$(".autowms_app_fileitem_return").attr("onclick",
"").click(function(ua){I=[];ea("")})},!0)},ea=function(Z){Z&&"undefined"!==Z||(Z="");var ma=$(".autowms_app_request_input").first().val();ma&&0<ma.length&&(z=ma.trim().split("?")[0],z.endsWith("?")||(z+="?"));var za=function(ua){if(ua.error)e.html("Error from server: "+ua.error);else{F={};I[I.length-1]!==Z&&I.push(Z);var Ba=aa(ua,Z);e.html("... working ... ");ua.result.sort(function(La,Oa){if(La.name<=Oa.name)return-1;if(La.name>Oa.name)return 1;if(La.name==Oa.name)return 0});for(var Na=0;Na<ua.result.length;Na++)Ba+=
N(ua.result[Na]),F[ua.result[Na].name]=ua.result[Na];e.html(Ba);$(".autowms_app_fileitem_return").attr("onclick","").click(function(La){I.pop();ea(I[I.length-1])});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(La){var Oa=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){ea(Oa)},100)});$(".autowms_app_fileitem").attr("onclick","").click(function(La){La=F[$(this).attr("name")];!1===La.leaf?ea(La.path):ca(ua,La.adaguc)});
$(".autowms_app_request_button").attr("onclick","").click(function(La){ea("")});$(".autowms_app_request_input").keyup(function(La){13==La.keyCode&&ea("")})}},va=z+"request=getfiles&path="+Z;e.html('... Reading <a href="'+va+'">'+va+"</a> ... ");try{$.ajax({contentType:"application/json",crossDomain:!0,type:"GET",url:va,success:za,error:function(ua){console.error("Request failed without jsonp, trying jsonp");za({result:[]});$.ajax({dataType:"jsonp",contentType:"application/jsonp",crossDomain:!0,type:"GET",
url:va,success:za,error:function(Ba){ja("Unable to do ajax call")}})}})}catch(ua){console.error(ua),ja(ua)}};this.enable=function(){e.html("<div>Initializing...</div>");ea("")};this.disable=function(){};this.resize=function(Z,ma){}};try{WMJSExtApplications.AutoWMS=autowms_app}catch(e){};
