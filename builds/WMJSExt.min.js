...
var c=location.href.split("?")[0];c=c.split("#")[0];-1!=c.indexOf("/index.html")&&(c=c.substring(0,c.indexOf("/index.html")),c+="/");-1!=c.indexOf("/index.jsp")&&(c=c.substring(0,c.indexOf("/index.jsp")),c+="/");-1!=c.indexOf("/index.php")&&(c=c.substring(0,c.indexOf("/index.php")),c+="/");c+="index.html";var e=a.mappanel.webMapJS,f=0,p=e.getActiveLayer(),m=e.getLayers();isDefined(void 0)&&(m=[],m.push(void 0));var k=e.getProjection();var l="?srs="+URLEncode(k.srs);l+="&bbox="+URLEncode(k.bbox);if(0<
m.length){var t=[],q=[];for(k=0;k<m.length;k++)if(m[k].name&&m[k].title!=WMJSEmptyLayerName){var g=t.indexOf(m[k].service);-1==g&&(t.push(m[k].service),g=t.length-1);q.push(m[k].name.replace(/,/g,".")+"$"+m[k].format+"$"+m[k].enabled+"$"+m[k].getStyle()+"$"+m[k].opacity+"$"+g);p==m[k]&&(f=k)}p=URLEncode(t[0]);for(k=1;k<t.length;k++)p+=","+URLEncode(t[k]);l+="&service="+p;p=URLEncode(q[0]);for(k=1;k<q.length;k++)p+=","+URLEncode(q[k]);l=l+("&layer="+p)+("&selected="+f);f=e.getDimensionList();isDefined(void 0)&&
(f=(void 0).dimensions);if(0<f.length)for(l+="&dims=",k=0;k<f.length;k++)0<k&&(l+=","),l+=f[k].name+"$"+f[k].currentValue}e.isMapPinVisible();e=e.getBaseLayers();f="";for(k=0;k<e.length;k++)e[k].enabled&&(0<f.length&&(f+="$"),f+=URLEncode(e[k].name));""==f&&(f="none");c+=l+("&baselayers="+f);b.setValue('<a href="'+c+'" target="_blank">'+c+" </a>")}}}});webmapjsext.WMJSExt.PermaLinkPanel.superclass.initComponent.apply(this,arguments)}});Ext.define("webmapjsext.WMJSExt.SpinnerSlider",{extend:"Ext.panel.Panel",alias:["widget.spinnerslider"],requires:["Ext.slider.Single"],initComponent:function(){var a=this;a.slidertitle=a.title;a.title=void 0;a.setValue=function(b){a.slider.suspendEvents();a.slider.setValue(b);a.slider.resumeEvents();a.label.setText(""+decimalFormatter(b,a.ndecs))};a.getValue=function(){return a.slider.getValue()};a.setValueByEvent=function(b){a.setValue(b);a.trigger&&a.trigger(b)};Ext.apply(this,{border:!1,height:15,
...
"dim_member"==key&&(u="*");"dim_ensemble_member"==key&&(u="*");"dim_scenario"==key&&(u="*");"dim_threshold"==key&&(u="*");"dim_percentile"==key&&(u="*");n+=key+"="+URLEncode(u)+"&"}$("#chart").append("<div class='chartstyle' id='chart"+v+"'></div>");$("#chart"+v).html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');m(n,"chart"+v)}$("#info").html(g)}else $("#info").html("No valid data received:<br/>"+data)}};b.addListener("mouseclicked",t,!0);this.enable=function(){a.html('<div id="info"></div><div id="chart" class="gfiapp_d3c3"></div>');
$("#info").html("Click on the map to create a timeseries graph.");console.log("init");b.enableInlineGetFeatureInfo(!1);k=!0};this.disable=function(){k=!1;b.enableInlineGetFeatureInfo(!0);l.set=!1};this.resize=function(q,g){console.log("w="+q);$("#chart").width(q);l.set&&t(l)};this.enable()};try{WMJSExtApplications["Time series mode"]=gfiapp_d3c3}catch(a){};var gfiapp_point_interest=function(a,b){var c=this,e=[],f=!0,p=function(m,k){var l=null,t=null,q=null;setflags=function(){l&&t&&q?(lon=l,lat=t,name=q,fullUrl=knmiFlagURL+"file="+pointInterestFileName+"&location="+lon+","+lat+"&name="+name,$.ajax({type:"GET",url:fullUrl,success:function(h){"OK"===h.status?(h=Ext.getCmp("webmapjs_add_point_interest"),h.getEl().dom.style.background="#BCED91",b.getImageStore().clear(),b.draw("draw")):(h=Ext.getCmp("webmapjs_add_point_interest"),h.getEl().dom.style.background=
