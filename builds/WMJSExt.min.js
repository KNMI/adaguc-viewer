...
p=Ext.create("Ext.data.Store",{fields:["title","srs","bbox"],data:{items:[]},proxy:{type:"memory",reader:{type:"json",root:"items"}}}),l=Ext.create("Ext.grid.Panel",{border:!1,store:p,columns:[{text:"Title",dataIndex:"title",flex:1},{text:"ESPG Code",dataIndex:"srs"}],listeners:{select:{fn:function(n,t){n=t.get("bbox");c=t.get("srs");f=n;u.getComponent("column2").getComponent("areaPanel").getComponent("bboxtop").setValue(n.split(",")[3]);u.getComponent("column2").getComponent("areaPanel").getComponent("bbox").getComponent("bboxwest").setValue(n.split(",")[0]);
u.getComponent("column2").getComponent("areaPanel").getComponent("bbox").getComponent("bboxeast").setValue(n.split(",")[2]);u.getComponent("column2").getComponent("areaPanel").getComponent("bboxbottom").setValue(n.split(",")[1]);t=Math.abs(f.split(",")[2]-f.split(",")[0]);n=Math.abs(f.split(",")[3]-f.split(",")[1]);var x=a.coverages[0];u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("xresolution").setValue(t/x.width);u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("yresolution").setValue(n/
x.height);r()}}}}),h=Ext.create("Ext.data.Store",{fields:["title","format"],data:{items:[]},proxy:{type:"memory",reader:{type:"json",root:"items"}}}),m=Ext.create("Ext.grid.Panel",{border:!1,store:h,columns:[{text:I18n.format.text,dataIndex:"format"},{text:I18n.title.text,dataIndex:"title",flex:1}],listeners:{select:{fn:function(n,t){e=t.get("format");r()}}}});a.onReady=function(){l.setLoading(!1);for(var n=this.coverages[0],t=0;t<n.supportedProjections.length;t++){var x=n.supportedProjections[t].srs;
try{x=epsgDescriptionLookup[n.supportedProjections[t].srs]}catch(y){}isDefined(x)||(x=n.supportedProjections[t].srs);p.add({title:x,srs:n.supportedProjections[t].srs,bbox:n.supportedProjections[t].bbox})}t=p.find("srs",n.nativeCRS);-1==t&&(t=p.find("srs","EPSG:4326"));-1==t&&(t=0);l.getView().select(t);p.add({title:"Viewer window projection and area",srs:this.srs,bbox:this.bbox});u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterwidth").setValue(n.width);
u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterheight").setValue(n.height);x=u.getComponent("column3").getComponent("dimensionPanel");for(t=0;t<n.dimensions.length;t++)x.add({xtype:"textfield",fieldLabel:n.dimensions[t].name,value:n.dimensions[t].currentValue,labelWidth:40,padding:"30",width:200,itemId:"dimtextfield_"+n.dimensions[t].name,listeners:{change:{fn:function(){r()}}}});for(t=0;t<n.formats.length;t++){x=n.formats[t];var w=n.formats[t].toUpperCase();
try{x=formatDescriptionLookup[w]}catch(y){}isDefined(x)||(x=n.formats[t]);h.add({title:x,format:n.formats[t]});"AAIGRID"==w&&m.getView().select(t)}r();v.update('<table style="margin:5px;"><tr><td><b>Coverage:</b></td><td>'+n.name+"</td></tr><tr><td><b>Title:</b></td><td>"+n.title+"</td></tr><tr><td><b>Description:</b></td><td>"+n["abstract"]+"</td></tr></table>")};Ext.create("Ext.panel.Panel",{region:"west",border:!0,width:100});var u=Ext.create("Ext.panel.Panel",{region:"center",border:!1,layout:{type:"hbox",
...
items:[q,v,k]});l.setLoading(!0);var r=function(){var n=a.coverages[0].service;n=WMJScheckURL(n);var t=a.coverages[0].name,x=u.getComponent("column2").getComponent("areaPanel").getComponent("bbox").getComponent("bboxwest").getValue()+","+u.getComponent("column2").getComponent("areaPanel").getComponent("bboxbottom").getValue()+","+u.getComponent("column2").getComponent("areaPanel").getComponent("bbox").getComponent("bboxeast").getValue()+","+u.getComponent("column2").getComponent("areaPanel").getComponent("bboxtop").getValue();
-1==n.indexOf("?")?n+="?":"&"!=n.charAt(n.length-1)&&(n+="&");n=n+"SERVICE=WCS&REQUEST=GetCoverage&COVERAGE="+t+"&CRS="+URLEncode(c)+"&FORMAT="+e+"&BBOX="+x;1==u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("radioselectcellsize").getValue()?(n+="&RESX="+u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("xresolution").getValue(),n+="&RESY="+u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("yresolution").getValue()):
(n+="&WIDTH="+u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterwidth").getValue(),n+="&HEIGHT="+u.getComponent("column2").getComponent("gridproperties").getComponent("field").getComponent("rasterheight").getValue());for(t=0;t<a.coverages[0].dimensions.length;t++)try{var w=u.getComponent("column3").getComponent("dimensionPanel").getComponent("dimtextfield_"+a.coverages[0].dimensions[t].name).getValue(),y=a.coverages[0].dimensions[t].name;y=y.toUpperCase();
...
resize:{fn:function(){b.applicationSettings.app&&b.applicationSettings.app.resize(b.width,mainWebmapJS.webMapJS.getHeight())}}},setApplication:function(g){try{b&&b.applicationSettings&&b.applicationSettings.app&&b.applicationSettings.app.disable()}catch(k){}b.applicationSettings=g}});a=Ext.create("Ext.panel.Panel",{region:"center",layout:"border",split:!0,frame:!1,border:!1,margin:0,items:[mainWebmapJS,b]});var c=function(g){var k=createNewLayerPanel(mainWebmapJS.webMapJS,{service:g.service,name:g.layer,
style:g.style,opacity:g.opacity,ready:function(v,r){var n=g.srs,t=g.bbox,x=g.baselayername,w=g.baselayerservice;n&&!t?r.setProjection(v.getProjection(n)):n&&t&&r.setProjection({srs:n,bbox:t});x&&w?(mainWebmapJS.baseLayer.setService(w),mainWebmapJS.baseLayer.name=x,mainWebmapJS.baseLayer.parseLayer(function(){r.draw("dataPanelClicked::parseLayer")},void 0,"WMJSExtMain::dataPanelClicked")):r.draw("dataPanelClicked");isDefined(k)&&isDefined(g.ready)&&(g.ready(k,r),1==k.WMJSLayer.hasError&&k.close())}});
return k},e,f=new Console;Ext.create("Ext.panel.Panel",{html:'<div id="debuginfodiv"></div>',layout:"fit",listeners:{afterrender:{fn:function(){f.setElement(document.getElementById("debuginfodiv"))}}}});var p=[{text:I18n.show_time_selection_window.text,iconCls:"button_settings_icon",handler:function(){timeselectorWindow.show()}},{text:I18n.create_animation.text+"&hellip;",iconCls:"button_settings_icon",handler:function(){Ext.create("Ext.window.Window",{title:I18n.create_animation.text,width:460,height:300,
...
handler:function(){mainWebmapJS.webMapJS.abort()}}];try{isDefined(getFeatureInfoApplications)}catch(g){getFeatureInfoApplications=[]}for(var l={},h=0;h<getFeatureInfoApplications.length;l={$jscomp$loop$prop$location$9$19:l.$jscomp$loop$prop$location$9$19},h++)l.$jscomp$loop$prop$location$9$19=getFeatureInfoApplications[h],p.push({text:getFeatureInfoApplications[h].name,iconCls:getFeatureInfoApplications[h].iconCls,location:l.$jscomp$loop$prop$location$9$19,handler:function(g){return function(){b.setApplication(g.$jscomp$loop$prop$location$9$19);
b.hide();b.show()}}(l)}),getFeatureInfoApplications.open&&l.$jscomp$loop$prop$location$9$19.name===getFeatureInfoApplications.open&&b.setApplication(l.$jscomp$loop$prop$location$9$19);p.push({text:I18n.about_the_adaguc_viewer.text,iconCls:"button_info",tooltip:I18n.about_the_adaguc_viewer.tooltip,handler:function(){Ext.create("Ext.window.Window",{title:I18n.about_adaguc.text,height:200,width:400,modal:!0,layout:"fit",items:{xtype:"panel",html:'<div class="about_adaguc" style="position:absolute;top:0px;left:0px;height:100px;width:520px;"/><div style="position:absolute;top:100px;padding:10px;margin:10px;">'+
...
viewportwestpanelwindow.y+viewportwestpanelwindow.height;g>u.height&&(g=u.height-70,70>g&&(g=70));viewportwestpanelwindow.setSize(viewportwestpanelwindow.width,g)}catch(k){}}}}}),q=function(g){var k=!1,v=!1;isDefined(g.collapse)&&"1"==g.collapse&&Ext.getCmp("viewportwestpanel").collapse();isDefined(g.zoomtolayer)&&"1"==g.zoomtolayer&&(k=!0);isDefined(g.srs)&&isDefined(g.bbox)&&mainWebmapJS.webMapJS.setProjection({srs:g.srs,bbox:g.bbox});if(isDefined(g.service)){clearLayers(mainWebmapJS.webMapJS);
var r=URLDecode(g.service).split(","),n=[""];isDefined(g.layer)?n=URLDecode(g.layer).split(","):1==k&&(v=!0);isDefined(g.selected);for(var t=n.length,x=0;x<n.length;x++)try{var w=n[n.length-1-x].split("$"),y=w+"",A=!0,z="",C=1,B=r[0]+"";6==w.length&&(y=w[0],A=w[2],z=w[3],C=w[4],isDefined(A)&&("true"===A&&(A=!0),"false"===A&&(A=!1)),B=r[w[5]]);createNewLayerPanel(mainWebmapJS.webMapJS,{service:B,name:y,style:z,enabled:A,opacity:C,ready:function(D,E){t--;if(0==t){if(isDefined(g.dims))for(var H=g.dims.split(","),
...
g.substring(g.indexOf("addlayer('")+10,g.indexOf("',")),g=g.substring(g.indexOf(",'")+2,g.indexOf("')")),k&&g){for(var v=!1,r=mainWebmapJS.webMapJS.getLayers(),n=0;n<r.length;n++)if(r[n].service==k&&r[n].name==g){v=!0;break}0==v&&(isDefined(hashLocationNotfiyAddLayer),firstTimeOpening=!0,createNewLayerPanel(mainWebmapJS.webMapJS,{service:k,name:g,zoomtolayer:!1,notifyadded:!0}))}}else 0<=g.indexOf("positionmappinbylatlon(")?(debug(g),k=g.substring(23,g.indexOf(")")),debug(k),k=k.split(","),debug(k[0]+
" "+k[1]),mainWebmapJS.webMapJS.positionMapPinByLatLon({x:parseFloat(k[0]),y:parseFloat(k[1])})):0<=g.indexOf("refreshlayers")?updateLayers():q(k)});getFeatureInfoApplications.open&&b.show()});var gfiapp_d3c3=function(a,b){var c=function(q,g){$("#"+g).empty();$("#"+g).append("<div style='height=240px;border=2px solid blue;' class='chart'>a</div>");$("#"+g).append("<div style='height=40px;border=2px solid red;' class='controls gfiapp_d3c3_export_to_csv_button'>b</div>");var k=$("#"+g).find(".chart");g=$("#"+g).find(".controls");k.html("graph");g.button({label:"Export to CSV"}).click(function(){console.log("Export to CSV");for(var v="",r=q.datatoplot,n=0;n<r[0].length;n++){for(var t=0;t<
r.length;t++)0<t&&(v+=";"),v+=JSON.stringify(r[t][n]);v+="\n\r"}r=document.createElement("a");r.setAttribute("download","export.csv");r.setAttribute("href",encodeURI("data:text/csv;charset=utf-8,"+v));console.log("Clicking link");document.body.appendChild(r);r.click();r.remove()});c3.generate({bindto:k.get(0),data:{x:"x",x_format:"%Y%m%d",columns:q.datatoplot,types:{x:"area"}},axis:{x:{type:"timeseries",tick:{format:q.timeFormat}},y:{label:q.ylabel}},zoom:{enabled:!0},subchart:{show:!1}})},e=function(q,
g){return q>g?1:q<g?-1:0},f=function(q,g){var k;for(k=0;k<g.length;k++)try{if(g[k].getTime()===q.getTime())return k}catch(v){}return-1},p=function(q,g){var k=1;"string"!=typeof q[0].dims&&(k=q[0].dims.length);for(var v=[],r=function(w,y,A){var z=["x"];w=[y.name+" "+w];for(var C in A){y=d3.time.format("%Y-%m-%dT%H:%M:%SZ").parse(C);var B=parseFloat(A[C]);z.push(y);w.push(B)}v.push({date:z,value:w})},n=function(w,y,A,z){if(1==k)r("",y,A);else if(w+2<k)for(d in A)n(w+1,y,A[d],z+"["+d+"]");else for(d in A)r("["+
d+"]",y,A[d])},t=0;t<q.length;t++)n(0,q[t],q[t].data,"");t=function(w){for(var y=[],A=w.length,z=0;z<A;z++)for(var C=w[z].date,B=1;B<C.length;B++){var D=C[B],E=f(D,y);-1==E&&y.push(D)}y.sort(e);y.unshift("x");C=[];for(B=0;B<A;B++)C.push([]);for(z=0;z<A;z++)for(C[z].push(w[z].value[0]),B=1;B<y.length;B++)E=f(y[B],w[z].date),E==B?(D=w[z].value[E],"nodata"==D&&(D=NaN),C[z].push(parseFloat(D))):C[z].push(NaN);w=[];w.push(y);for(z=0;z<A;z++)w.push(C[z]);return w}(v);var x=q[0].name;q[0].units&&(x+=" in "+
q[0].units);c({datatoplot:t,ylabel:x,timeFormat:"%Y-%m-%d %H:%M"},g)},l=function(q,g){$.ajax({type:"GET",url:q+"&JSONP=?",dataType:"jsonp",cache:!1,success:function(k){try{p(k,g)}catch(v){$("#"+g).html("Sorry! An error occurred while parsing JSON data: ["+v+"]")}},error:function(k,v,r){$("#"+g).html("Sorry! An error occurred while receiving JSON data from the server: ["+r+"]");console.log("Error "+r)}})},h=!0,m=[];m.set=!1;var u=function(q){if(0!=h){m.x=q.x;m.y=q.y;m.set=!0;var g=b.getLatLongFromPixelCoord({x:q.x,
y:q.y});$("#chart").html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');var k=b.getLayers();if(isDefined(k)){g="timeseries graph for location ["+Math.round(100*g.x)/100+","+Math.round(100*g.y)/100+"]<br/>";$("#chart").empty();for(var v=0;v<k.length;v++){var r=b.getWMSGetFeatureInfoRequestURL(k[v],q.x,q.y);r=composeUrlObjectFromURL(r);var n=r.location+"?";for(key in r.kvp){var t=r.kvp[key];"info_format"==key&&(t="application/json");"time"==key&&(t="1000-01-01T00:00:00Z/3000-01-01T00:00:00Z");
"dim_member"==key&&(t="*");"dim_ensemble_member"==key&&(t="*");"dim_percentile"==key&&(t="*");n+=key+"="+URLEncode(t)+"&"}$("#chart").append("<div class='chartstyle' id='chart"+v+"'></div>");$("#chart"+v).html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');l(n,"chart"+v)}$("#info").html(g)}else $("#info").html("No valid data received:<br/>"+data)}};b.addListener("mouseclicked",u,!0);this.enable=function(){a.html('<div id="info"></div><div id="chart" class="gfiapp_d3c3"></div>');$("#info").html("Click on the map to create a timeseries graph.");
...
tooltip:I18n.delete_selected_flag_tooltip.tooltip,renderTo:"webmapjs_div_selected_point_interest",cls:"webmapjs_button_delete_point_interest",handler:function(){delete_flag()}});$("#webmapjs_point_interest_name, #webmapjs_point_interest_lon, #webmapjs_point_interest_lat").keypress(function(k){if(13==k.which)return setflags(),!1});var g=function(){var k=a.find(".gfiapp_point_interest");k.empty();for(var v=b.getLayers(),r=0;r<v.length;r++){var n=v[r],t=jQuery("<div />",{css:{margin:8}});k.append(t);
t.html("LAYER "+n.service)}c.resize(l,h)};g();b.addListener("onlayeradd",g,!0);b.addListener("mouseclicked",function(k){if(0!=f){var v=b.getLatLongFromPixelCoord({x:k.x,y:k.y});m=v.x;u=v.y;$("#webmapjs_point_interest_lon").val(v.x.toFixed(5));$("#webmapjs_point_interest_lat").val(v.y.toFixed(5));$("#webmapjs_point_interest_lon, #webmapjs_point_interest_lat").css("border","1px solid grey");v=b.getLayers();k=b.getWMSGetFeatureInfoRequestURL(v[0],k.x,k.y).replace("&INFO_FORMAT=text/html","&INFO_FORMAT=application/json")+
"&JSONP=featureInfoCallback";$.ajax({dataType:"jsonp",jsonpCallback:"featureInfoCallback",type:"GET",cache:!1,url:k,success:function(r){r[1]&&"nodata"!==r[1].data?($(".webmapjs_label_selected_point_interest").text(r[1].data),$(".webmapjs_button_delete_point_interest").css("display","inline")):($(".webmapjs_label_selected_point_interest").text(I18n.default_selected_flag.text),$(".webmapjs_button_delete_point_interest").css("display","none"))},error:function(r,n,t){debug(n+", "+t)}})}},!0);$("#webmapjs_point_interest_name").on("input",
function(k){onChangeEvents("name",k)});$("#webmapjs_point_interest_lon").on("input",function(k){onChangeEvents("lon",k);changeMapPin()});$("#webmapjs_point_interest_lat").on("input",function(k){onChangeEvents("lat",k);changeMapPin()});this.onChangeEvents=function(k,v){var r=Ext.getCmp("webmapjs_add_point_interest");r.getEl().dom.style.background="";r=Ext.getCmp("webmapjs_delete_point_interest");r.getEl().dom.style.background="";switch(k){case "name":q=v.target.value;break;case "lon":m=v.target.value;
...
g!=q&&k.draw();k.resumeEvent("onloadingcomplete");k.resumeEvent("onupdatebbox")}},u=function(){var q=a.find(".gfiapp_eprofile");q.empty();for(var g=b.getLayers(),k=0;k<g.length;k++){var v=g[k],r=jQuery("<div />",{css:{margin:8}});q.append(r);e[k]=new GFITimeElevationWindow(r);e[k].getWebMapJS().addLayer(v);e[k].getWebMapJS().getListener().id=k;e[k].getWebMapJS().addListener("onupdatebbox",h,!0);e[k].getWebMapJS().addListener("onloadingcomplete",m,!0)}c.resize(p,l)};u();b.addListener("ondimchange",
...
!0);b.enableInlineGetFeatureInfo(!1)};this.enable=function(p,l){f(p,l)};this.disable=function(){b.enableInlineGetFeatureInfo(!0)};this.resize=function(p,l){for(var h=0;h<e.length;h++)e[h].setSize(p,l/e.length);for(h=0;h<e.length;h++);};c.enable()};try{WMJSExtApplications.EProfile=gfiapp_eprofile}catch(a){};var gfiapp_histogram=function(a,b){var c=function(h,m){$.ajax({type:"GET",url:h+"&JSONP=?",dataType:"jsonp",cache:!1,success:function(u){try{var q=Object.keys(u)[0],g=u[q];$("#"+m).empty();$("#"+m).append("<div style='height=240px;border=2px solid blue;' class='chart'>a</div>");var k=$("#"+m).find(".chart");k.html("graph");var v=[["x"].concat($jscomp.arrayFromIterable(g.interval)),[q].concat($jscomp.arrayFromIterable(g.quantity))];c3.generate({bindto:k.get(0),data:{x:"x",columns:v}})}catch(r){console.error(r),
$("#"+m).html("Sorry! An error occurred while parsing JSON data: ["+r+"]")}},error:function(u,q,g){$("#"+m).html("Sorry! An error occurred while receiving JSON data from the server: ["+g+"]");console.log("Error "+g)}})},e=!0,f=[];f.set=!1;var p=function(h){if(0!=e){f.set=!0;$("#chart").html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');var m=b.getLayers();if(isDefined(m)){var u="Histogram graph for area ["+Math.round(100*h.left)/100+","+Math.round(100*h.bottom)/100+","+Math.round(100*h.right)/
100+","+Math.round(100*h.top)/100+"]<br/>";$("#chart").empty();for(var q=0;q<m.length;q++){var g=b.buildWMSGetMapRequest(m[q]);g=composeUrlObjectFromURL(g);var k=g.location+"?";for(key in g.kvp){var v=g.kvp[key];"request"==key&&(v="GetHistoGram");if("bbox"==key){v=g.kvp.crs;var r=!1;"1.3.0"===g.kvp.version&&"EPSG:4326"===v&&(r=!0);v=r?h.bottom+","+h.left+","+h.top+","+h.right:h.left+","+h.bottom+","+h.right+","+h.top}k+=key+"="+URLEncode(v)+"&"}$("#chart").append("<div class='chartstyle' id='chart"+
q+"'></div>");$("#chart"+q).html('<img src="./img/ajax-loader.gif" alt="Loading..."/>');c(k,"chart"+q)}$("#info").html(u)}else $("#info").html("No valid data received:<br/>"+data)}},l=function(){a.html('<div id="info"></div><div id="chart" class="gfiapp_histogram"></div>');$("#info").html("Click on the map to create a histogram graph.");console.log("init");b.enableInlineGetFeatureInfo(!1);b.setMapModeZoomBoxIn();b.addListener("beforezoomend",function(h){p(h);return!1},!0)};this.enable=function(){l();
e=!0};this.disable=function(){console.log("disable");e=!1;b.enableInlineGetFeatureInfo(!0);b.setMapModePan();b.removeListener("beforezoomend");b.divZoomBox.style.display="none";f.set=!1};this.resize=function(h,m){$("#chart").width(h);f.set&&p(f)};this.enable()};try{WMJSExtApplications["Histogram mode"]=gfiapp_histogram}catch(a){};var autowms_app=function(a,b){initWMJS();a.addClass("autowms_app_container");var c="https://geoservices.knmi.nl/autowms?";try{c=autowmsURL}catch(g){}var e={},f=[],p=function(g){var k=m();a.html(k+JSON.stringify(g));$(".autowms_app_fileitem_return").attr("onclick","").click(function(v){f=[];q("")});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(v){var r=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){q(r)},100)})},
...
(k+'">&#8635;&nbsp;<b>Refresh</b></span></span>')},u=function(g,k){a.html("... working ... ");var v=WMJSGetServiceFromStore(k,xml2jsonrequestURL);v.getLayerObjectsFlat(function(r){var n=""+m(g,k);n+="<span class='autowms_app_serviceabstract'><b>"+v.title.replace(/!/g,"<br/>")+"</b><hr/>"+v.abstract.replace(/!/g,"<br/>");if(k&&(n+='<hr/>WMS: <a target="_blank" href="'+k+'&service=WMS&request=GetCapabilities">'+k+"</a>",1<k.split("?").length)){var t=new WMJSKVP(k.split("?")[1]),x=k.split("?")[0].replace("adagucserver",
"adagucopendap/");x=x.replace(/([^:]\/)\/+/g,"$1");t.source&&(x+=t.source);t.dataset&&(x+=t.dataset+"/");n=n+('<hr/>OpenDAP: <span class=\'autowms_app_opendaplinks\'><a target="_blank" href="'+x+'">'+x+'</a><a target="_blank" href="')+(x+'.das">DAS</a><a target="_blank" href="')+(x+'.dds">DDS</a></span>')}n+="</span>";a.html("... working ... ");e={};for(t=0;t<r.length;t++)n+=h(r[t],k),e[r[t].name]=r[t];a.html(n);$(".autowms_app_layeritem").attr("onclick","").click(function(w){w=e[$(this).attr("name")];
...
function(r){r=m();r=r+"<span class='autowms_app_error_container'><span class='autowms_app_error_container_header'>Unable to get WMS GetCapabilities</span><a target='_blank' href=\""+(k+'service=WMS&request=GetCapabilities">'+k+"</a>");a.html(r+"</span>");$(".autowms_app_fileitem_return").attr("onclick","").click(function(n){f=[];q("")})},!0)},q=function(g){g&&"undefined"!==g||(g="");var k=$(".autowms_app_request_input").first().val();k&&0<k.length&&(c=k.trim().split("?")[0],c.endsWith("?")||(c+="?"));
var v=function(n){if(n.error)a.html("Error from server: "+n.error);else{e={};f[f.length-1]!==g&&f.push(g);var t=m(n,g);a.html("... working ... ");n.result.sort(function(w,y){return w.name.localeCompare(y.name)});n.result.sort(function(w,y){if(w.leaf===y.leaf)return 0;if(!0===w.leaf)return 1;if(!0===y.leaf)return-1});for(var x=0;x<n.result.length;x++)t+=l(n.result[x]),e[n.result[x].name]=n.result[x];a.html(t);$(".autowms_app_fileitem_return").attr("onclick","").click(function(w){f.pop();q(f[f.length-
1])});$(".autowms_app_fileitem_refresh").attr("onclick","").click(function(w){var y=$(this).attr("name");$(".autowms_app_fileitem").remove();$(".autowms_app_layeritem").remove();setTimeout(function(){q(y)},100)});$(".autowms_app_fileitem").attr("onclick","").click(function(w){w=e[$(this).attr("name")];!1===w.leaf?q(w.path):u(n,w.adaguc)});$(".autowms_app_request_button").attr("onclick","").click(function(w){q("")});$(".autowms_app_request_input").keyup(function(w){13==w.keyCode&&q("")})}},r=c+"request=getfiles&path="+
g;a.html('... Reading <a href="'+r+'">'+r+"</a> ... ");try{$.ajax({contentType:"application/json",crossDomain:!0,type:"GET",url:r,success:v,error:function(n){console.error("Request failed without jsonp, trying jsonp");v({result:[]});$.ajax({dataType:"jsonp",contentType:"application/jsonp",crossDomain:!0,type:"GET",url:r,success:v,error:function(t){p("Unable to do ajax call")}})}})}catch(n){console.error(n),p(n)}};this.enable=function(){a.html("<div>Initializing...</div>");q("")};this.disable=function(){};
