...
export var WMJSCoverage = function (options) {
  this.name = undefined;
...
export var WCJSRequest = function (service, name, succes, fail) {
  ...
  var url =
    service +
    "&service=WCS&REQUEST=DescribeCoverage&COVERAGE=" +
    URLEncode(name);
  // alert(url);
  ...
  var errormessage = function (message) {
    fail(
      I18n.unable_to_do_getcapabilities.text +
        ":\n" +
        getcapreq +
        "\n" +
        I18n.result.text +
  ...
  console.log(getcapreq);

  WMJSXMLParser(getcapreq)
    .then((data) => {
      try {
        _succes(data);
      } catch (e) {
...
export var parseDescribeCoverage = function (jsonDoc, _coverage) {
  var coverage;
  if (_coverage) {
    coverage = _coverage;
  } else {
  ...
  coverage["abstract"] =
    jsonDoc.CoverageDescription.CoverageOffering.description.value;
  var lonLatEnvelope1 =
    jsonDoc.CoverageDescription.CoverageOffering.lonLatEnvelope[
      "gml:pos"
    ][0].value.replaceAll(" ", ",");
  var lonLatEnvelope2 =
    jsonDoc.CoverageDescription.CoverageOffering.lonLatEnvelope[
  ...
  var spatialDomain = toArray(
    jsonDoc.CoverageDescription.CoverageOffering.domainSet.spatialDomain[
      "gml:Envelope"
    ]
  );
  ...
  var rectifiedGrid =
    jsonDoc.CoverageDescription.CoverageOffering.domainSet.spatialDomain[
      "gml:RectifiedGrid"
    ]["gml:limits"]["gml:GridEnvelope"]["gml:high"].value.split(" ");
  coverage.width = parseInt(rectifiedGrid[0]) + 1;
  coverage.height = parseInt(rectifiedGrid[1]) + 1;

  coverage.cellsizeX = parseFloat(
    jsonDoc.CoverageDescription.CoverageOffering.domainSet.spatialDomain[
      "gml:RectifiedGrid"
    ]["gml:offsetVector"][0].value.split(" ")[0]
  );
  coverage.cellsizeY = parseFloat(
    jsonDoc.CoverageDescription.CoverageOffering.domainSet.spatialDomain[
      "gml:RectifiedGrid"
    ]["gml:offsetVector"][1].value.split(" ")[1]
  );

  coverage.originX =
    jsonDoc.CoverageDescription.CoverageOffering.domainSet.spatialDomain[
      "gml:RectifiedGrid"
    ]["gml:origin"]["gml:pos"].value.split(" ")[0];
  coverage.originY =
    jsonDoc.CoverageDescription.CoverageOffering.domainSet.spatialDomain[
      "gml:RectifiedGrid"
    ]["gml:origin"]["gml:pos"].value.split(" ")[1];

  if (jsonDoc.CoverageDescription.CoverageOffering.supportedCRSs.nativeCRSs) {
    coverage.nativeCRS =
      jsonDoc.CoverageDescription.CoverageOffering.supportedCRSs.nativeCRSs.value;
  } else {
    coverage.nativeCRS = "EPSG:4326";
  }

  coverage.nativeFormat =
    jsonDoc.CoverageDescription.CoverageOffering.supportedFormats.attr.nativeFormat;

  var formats = toArray(
    jsonDoc.CoverageDescription.CoverageOffering.supportedFormats.formats
  );
