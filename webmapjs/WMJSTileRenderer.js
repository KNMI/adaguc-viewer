  ...
  render(
    ...
    let drawBGTiles = (level) => {
      ...
      let numTilesAtLevelX =
        tileSetWidth / ((initialResolution / numTilesAtLevel) * tileSize); // / Math.abs(originShiftY / originShiftX);
      let numTilesAtLevelY =
        tileSetHeight / ((initialResolution / numTilesAtLevel) * tileSize);
      let tilenleft = parseInt(
        Math.round(
      ...
      let drawTile = function (ctx, level, x, y, loadImage = true) {
        ...
        let bl = getPixelCoordFromGeoCoord(
          { x: bounds.left, y: bounds.bottom },
          newBBOX,
          width,
          height
        );
        let tr = getPixelCoordFromGeoCoord(
        ...
        if (tileServerType === "osm") {
          if (tmsEnabled) {
            ...
            imageURL =
              home + level + "/" + x + "/" + y + "." + tileServerFormat;
          }
        } else if (
          ...
          if (image.isLoaded()) {
            try {
              ctx.drawImage(
                ...
                parseInt(bl.x),
                parseInt(bl.y),
                parseInt(tr.x - bl.x) + 1,
                parseInt(tr.y - bl.y) + 1
              );
            } catch (e) {}
          } else {
      ...
      if (tilentop - tilenbottom > 20) {
        console.error("Too many tiles in vertical", tilentop - tilenbottom);
        return;
      }
      if (tilenright - tilenleft > 20) {
        console.error("Too many tiles in horizontal", tilentop - tilenbottom);
        return;
      }
