Ext.define("webmapjsext.WMJSExt.PermaLinkPanel", {
  ...
  initComponent: function () {
    ...
    var WMJSMakePermaLink = function (webMapJS, wmjsLayer) {
      ...
      if (mapLayers.length > 0) {
        ...
        for (j = 0; j < mapLayers.length; j++) {
          if (mapLayers[j].name && mapLayers[j].title != WMJSEmptyLayerName) {
            ...
            layers.push(
              mapLayers[j].name.replace(/,/g, ".") +
                "$" +
                mapLayers[j].format +
                "$" +
                mapLayers[j].enabled +
                "$" +
                mapLayers[j].getStyle() +
                "$" +
                mapLayers[j].opacity +
                "$" +
                serviceIndex
      ...
      for (var i = 0; i < baseLayers.length; i++) {
        if (baseLayers[i].enabled) {
          if (baseLayerName.length > 0) baseLayerName += "$";
          baseLayerName += URLEncode(baseLayers[i].name);
        }
      }
      if (baseLayerName == "") baseLayerName = "none";
    ...
    Ext.apply(this, {
          ...
          items: [
            {
              ...
              height: 28,
              region: "north",
              html: "<h1>Paste the link below in an email or chat message</h1>",
            },
            {
              xtype: "displayfield",
              region: "center",
              itemId: "mainTextField",
            },
          ],
        },
      ],
      listeners: {
        afterrender: {
          fn: function () {
            var mainTextField = _this
              .getComponent(0)
              .getComponent("mainTextField");
            var link =
              WMJSGetViewerLocation() +
              WMJSMakePermaLink(_this.mappanel.webMapJS);
    ...
    webmapjsext.WMJSExt.PermaLinkPanel.superclass.initComponent.apply(
      this,
      arguments
    );
  },
